...
def loader():
    ...
    #AstPlayer.query.delete() Lets try something different
    insert_players()
...
def insert_players(): 
    ...
    players_in_session = [player for player in players if ((player.created.weekday() in [5, 6]) and ((player.created.hour >= 18) or (player.created.hour <= 5)) and (player.is_jip == False) and (player.player_name not in ["HC", "Error: No unit", "Flash", "Namuthewhale", "SkinnyTar", "Sakai", "ratcatcher", "Tegyr", "Ricky", "ZeRandomTigre", "Wrathz", "Emaharg", "Pr3sario", "Shalun (2)", "Lucky"]))] # will contain players objects from sessions - list 
    ...
    leadership_roles = ["CO","SL","FTL","MMGG","MATG","HATG","SN","ENG","VC"]
    ...
    for player in players_in_session:
        ...
        if (player.player_name not in player_name_in_database):  # Since the player was in the session, lets see if he is new or not
            ...
            temp_player = AstPlayer(player_name=player.player_name, player_uid=player.player_uid, missions_played=1, last_played=player.created, danger_zone=False, player_rank="Regular", deaths=0, played_leader=0, last_mission=player.mission_id, staff_notes="") # create new player
            db.session.add(temp_player)
        else: # Stuff that needs to be refreshed because we could have new data
